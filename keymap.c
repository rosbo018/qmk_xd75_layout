#include QMK_KEYBOARD_H
/* initially a file converted from json */
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT_ortho_5x15(KC_ESC, KC_1, KC_2, KC_3, KC_4, KC_5, KC_MINS, KC_GRV, KC_EQL, KC_6, KC_7, KC_8, KC_9, KC_0, KC_BSPC,
			    KC_TAB, KC_Q, KC_W, KC_F, KC_P, KC_B, KC_LBRC, KC_BSLS, KC_RBRC, KC_J, KC_L, KC_U, KC_Y, KC_SCLN, KC_QUOT,
			    KC_CAPS, KC_A, KC_R, KC_S, KC_T, KC_G, KC_HOME, KC_DEL, KC_PGUP, KC_K, KC_N, KC_E, KC_I, KC_O, KC_ENT,
			    KC_LSFT, KC_Z, KC_X, KC_C, KC_D, KC_V, KC_END, KC_UP, KC_PGDN, KC_M, KC_H, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT,
			    KC_LCTL, KC_LGUI, KC_LALT, OSL(1), KC_LCTL, KC_LALT, KC_LEFT, KC_DOWN, KC_RGHT, KC_LGUI, KC_SPC, MO(1), KC_RALT, KC_RGUI, KC_RCTL),
    
    [1] = LAYOUT_ortho_5x15(KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_NLCK, KC_SLSH, KC_ASTR, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12,
			    KC_MSEL, KC_CALC, KC_MYCM, KC_MAIL, RGB_HUD, RGB_HUI, KC_P7, KC_P8, KC_P9, KC_MINS, KC_TRNS, KC_TRNS, KC_PSCR, KC_SLCK, KC_PAUS,
			    KC_MPRV, KC_MPLY, KC_MNXT, KC_MSTP, RGB_SAD, RGB_SAI, KC_P4, KC_P5, KC_P6, KC_PLUS, KC_TRNS, RESET, KC_TRNS, KC_TRNS, KC_TRNS,
			    KC_VOLD, KC_MUTE, KC_VOLU, KC_APP, RGB_VAD, RGB_VAI, KC_P1, KC_P2, KC_P3, KC_PENT, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
			    KC_TRNS, KC_TRNS, RGB_TOG, KC_TRNS, RGB_RMOD, RGB_MOD, KC_P0, KC_TRNS, KC_PDOT, KC_PENT, KC_PENT, MO(1), KC_TRNS, KC_TRNS, KC_TRNS)
};
